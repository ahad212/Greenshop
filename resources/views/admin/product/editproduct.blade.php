...
<style>
    ...
    <div class="container-fluid px-4">
        ...
        <form id="forsubmit" enctype="multipart/form-data">
            ...
            <div class="row row-cols-1 row-cols-lg-2">
                <div class="col">
                    <label for="cname" class="d-block" style="font-weight: bold;">
                        Product Name
                        <input type="text" id="cname" value="{{$product->name}}" class="form-control" name="pname" required>
                    </label>
                    <label for="ss" style="font-weight:bold;margin-top:5px;">Select Category</label>
                    <select name="scat" id="sss" class="form-control">
                        <option value="">Select Category</option>
                        @foreach ($category as $cat)
                            <option value="{{$cat->id}}" <?php if ($cat->id == $product->category)echo 'selected'; ?>>{{$cat->cname . ' ('. $cat->parent.')'}}</option>
                        @endforeach
                    </select>
                    ...
                    <label for="cdescription" class="d-block" style="font-weight: bold;margin-top:5px;">
                        ...
                        <textarea cols="30" rows="5" id="cdescription" class="form-control" name="bdescription" placeholder="OS: Android 10;" required>{{$product->pdescription}}</textarea>
                    </label>
                    <label for="longgd" style="font-weight: bold;margin-top:5px;"> Description
                        <textarea name="longd" id="longgd" cols="30" rows="10" class="form-control">{{$product->longd}}</textarea>
                    </label>
                    <label for="quantity" class="d-block" style="font-weight: bold;">
                        Product Quantity
                        <input type="number" value="{{$product->quantity}}" id="quantity" class="form-control" name="quantity" required>
                    </label>
                </div>
                <div class="col">
                    <div class="custom-file">
                        ...
                        @php
                            $image_loop = json_decode($product->pimage);
                        @endphp
                        <style>
                            .spanimg{
                                border: 1px solid black;padding:1px 20px;cursor:pointer;margin-left:5px;
                            }
                        </style>
                        {{-- show logo --}}
                        <div id="showimg" style="margin-top:10px; border:1px solid black;">
                            {{-- images loop injected here --}}
                        </div>
                        <a class="btn btn-primary" onclick="increaseimage()">+<a>
                    </div>
                    ...
                    <label for="dprice" style="margin-top:5px;font-weight:bold;" class="d-block" style="font-weight: bold;">
                        Discount Price
                        <input type="text" value="{{$product->discount_price}}" name="dprice" id="dprice" class="form-control">
                    </label>
                    ...
                    {{-- <div class="status_wrapping">
                        ...
                        <div class="check_wrapping" style="display: flex;grid-column-gap:10px;">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="fy" value="yes" id="f1" <?php if($product->featured == 'yes') echo 'checked'?>>
                                <label class="form-check-label" for="f1">
                                    Yes
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="fy" value="no" id="f2" <?php if($product->featured == 'no') echo 'checked'?>>
                    ...
                    <div class="status_wrapping">
                        ...
                        <div class="check_wrapping" style="display: flex;grid-column-gap:10px;">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="sactive" value="active" id="flexRadioDefault1" <?php if($product->pstatus == 'active') echo 'checked'?>>
                                <label class="form-check-label" for="flexRadioDefault1">
                                    Active
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" value="inactive" name="sactive" id="flexRadioDefault2" <?php if($product->pstatus == 'inactive') echo 'checked'?>>
                    ...
                    <div class="status_wrapping">
                        ...
                        <div class="check_wrapping" style="display: flex;grid-column-gap:10px;">
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="radio" value="no" name="ia" id="imei2" <?php if($product->pimei == 'no') echo 'checked'?>>
                    ...
                    <div class="status_wrapping">
                        ...
                        <div class="check_wrapping" style="display: flex;grid-column-gap:10px;">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="ai" value="yes" id="warranty1" <?php if($product->pwarranty == 'yes') echo 'checked'?>>
                                <label class="form-check-label" for="warranty1">
                                    Yes
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" value="no" name="ai" id="warranty2" <?php if($product->pwarranty == 'no') echo 'checked'?>>
                    ...
                    {{-- change id for don't wanna hide here this field --}}
                    <div class="wainfo" id="wainfod"> 
                        <label for="win" style="font-weight: bold;">Warranty time period?</label>
                        <input type="text" class="form-control" id="win"  name="wamount" placeholder="e.g. 1 year" value="{{$product->warrantytime}}">
                    </div>
                    {{-- <h4>Color variant</h4>
                    ...
                    <input type="text" placeholder="Red,Green,Blue" id="variant" value="{{$product->color}}" name="variant" class="form-control">
                    <label for="size" class="d-block" style="font-weight: bold;">Size (separte size with ,)</label>
                    <input type="text" placeholder="14 inchs,22 inchs" value="{{$product->size}}" id="size" name="size" class="form-control">
                    <br>
            ...
            {{-- <a class="btn btn-primary" onclick="lastfinish()">test</a> --}}
        </form>
    ...
    <script>
    localStorage.setItem('qval','');
    let qvalue = document.getElementById('quantity').value;
    let quantityvaluesave = localStorage.setItem('qval',qvalue);
    function checkquntity2() {
        ...
        let getquantity = localStorage.getItem('qval');
    ...
    const productId = window.location.pathname.substr(31);
    let productImages=[];
    async function calldetails() {
        let formdata = new FormData();
        formdata.append('id',productId);
        await axios.post('/laraecomm/api/v1/singleproductedit',formdata).then(res=>{
            productImages = JSON.parse(res.data.pimage);
        });

        ediableimagesloop();
    }
    function ediableimagesloop() {
        let all = '';
    ...
    calldetails();
    let imageObjectFileArray = [];
    function increaseimage() {
        productImages.push('');
        ediableimagesloop();
    }
    function imgobjarray(event,index) {
        const ind = 'a'+index;

        imageObjectFileArray[ind] = event.files[0];
        productImages[index] = '/productimages/'+event.files[0].name;
        document.getElementById(ind).innerHTML = '/productimages/'+event.files[0].name;

    }
    function deleteimage(id) {
        productImages.splice(id,1);
        const ind = 'a'+id;
        delete imageObjectFileArray[ind];
        ediableimagesloop();
    }

    function lastfinish() {
        productImages;
        imageObjectFileArray;
        console.log(productImages);
        console.log(imageObjectFileArray);
    }
...
document.getElementById('forsubmit').addEventListener('submit',async function (event) {
    ...
    for (let key in imageObjectFileArray) {
        const element = imageObjectFileArray[key];
        formdata.append('image['+ key +']',element);
        console.log('element'+key+' :',element);
    }
    formdata.append('immageArray',JSON.stringify(productImages));
    formdata.append('id',productId);
