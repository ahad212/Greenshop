...
@section('main_content')
    ...
    <form action="{{route('editbrand',$getbrand->id)}}" method="POST" style="width:90%;margin:auto;" enctype="multipart/form-data">
        @csrf
        <div class="row row-cols-1 row-cols-lg-2">
            <div class="col">
                <label for="cname" class="d-block"  style="font-weight:bold;">
                    Brand Name
                    <input type="text" id="cname" value="{{$getbrand->bname}}" class="form-control" name="bname" >
                </label>
                <label for="cdes" class="d-block"  style="font-weight:bold;">
                    Brand Description
                    <textarea cols="30" rows="5" id="cdes" class="form-control" name="bdescription" >{{$getbrand->bdescription}}</textarea>
                </label>
                ...
                <div class="status_wrapping">
                    ...
                    <div class="check_wrapping" style="display: flex;grid-column-gap:10px;">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="fy" value="yes" id="f1" <?php if($getbrand->bfeatured == 'yes') echo 'checked'?>>
                            <label class="form-check-label" for="f1">
                                Yes
                            </label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="fy" value="no" id="f2" <?php if($getbrand->bfeatured == 'no') echo 'checked'?>>
